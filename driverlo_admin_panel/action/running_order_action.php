<?php
$search_type = $search_value = $where =  $new_order_list = $pagination = $total_row = $driver_name = $driver_license_no =  $booking_start_datetime = $booking_end_datetime  = "";
if(loader_post_isset('search_type')){
	...
	if(("" != $search_value)||("" != $booking_end_datetime))
	{
		if("mobile_no" == $search_type){
			$where = "WHERE mobile_no LIKE '%".$search_value."%'  AND is_approved = '1' AND is_completed = '0' AND is_cancelled = '0' ";
		}elseif("brn_no" == $search_type){
			$where = "WHERE brn_no LIKE '%".$search_value."%' AND is_approved = '1' AND is_completed = '0' AND is_cancelled = '0' ";
			...
			$where = "WHERE booking_datetime > '".$booking_start_datetime1."' 
			          AND booking_datetime < '".$booking_end_datetime1."' AND is_approved = '1' AND is_completed = '0' AND is_cancelled = '0' ";
		}
	}else{
		$where = "WHERE is_approved = '1' AND is_completed = '0' AND is_cancelled = '0' ";
	}
	...
	else
	{
		 ...
		 if("" == $case) 
	     {
			...
			{
				if("mobile_no" == $search_type){
					$where = "WHERE mobile_no LIKE '%".$search_value."%' AND is_approved = '1' AND is_completed = '0' AND is_cancelled = '0' ORDER BY brn_no DESC LIMIT ".$start_from .",".$rec_page." ";
				}elseif("brn_no" == $search_type){
					$where = "WHERE brn_no LIKE '%".$search_value."%' AND is_approved = '1' AND is_completed = '0' AND is_cancelled = '0' ORDER BY brn_no DESC LIMIT ".$start_from .",".$rec_page." ";
				}elseif("booking_datetime" == $search_type){
					$where =  "WHERE booking_datetime > '".$booking_start_datetime1."' AND booking_datetime < '".$booking_end_datetime1."'
							 AND is_approved = '1'   AND is_completed = '0' AND is_cancelled = '0' ORDER BY brn_no DESC LIMIT ".$start_from .",".$rec_page." ";
				}
			}
			else
			{
				$where = "WHERE is_approved = '1' AND is_completed = '0' AND is_cancelled = '0' ORDER BY brn_no DESC LIMIT ".$start_from .",".$rec_page."  ";
			}
		    $query_job_listing = "SELECT brn_no, mobile_no, customer_name, booking_datetime, pickup_point, booking_ai_id, driver_name, driver_license_no FROM view_booking_detail ".$where." ";
			...
			if($result)
			{
					while($rowdata = loader_fetch_assoc($result))
					{
							...
							$driver_license_no = $rowdata['driver_license_no'];
							$new_order_list .=   '<tr>
                                            <td>'.$brn_no.'</td>
											<td>'.$customer_name.'</td>
                                            <td>'.$mobile_no.'</td>
                                            ...
                                            <td>
                                                <div class="controls center">
                                                    <a href="'.ROOT_PATH.'edit_running_order/'.$booking_ai_id.'"  class="tip"><span class="icon12 icomoon-icon-pencil"></span></a>
													<a href="'.ROOT_PATH.'delete_order/'.$booking_ai_id.'"  class="tip"><span class="icon12 icomoon-icon-remove"></span></a>
													<a href="'.ROOT_PATH.'generate_bill/'.$booking_ai_id.'"  class="tip"><span class="icon12  icomoon-icon-copy-2"></span></a>
                                                </div>
                                            </td>
                                          </tr>';
						 $i++;				  
