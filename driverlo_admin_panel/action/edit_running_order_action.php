<?php
$session = $driver = $booked_driver_id = $name = $mobile_no = $hour = $minute =  $pickup_point= $dropoff_point = $pickup_lat = $pickup_lng =  $journey_type = $vehicle_mode = $estimated_time = $vehicle_type = $booking_datetime = $hour = $minute = $user_token = $coupon_code = $error_message = $success_message =  $is_active = $id = "";
//
		...
		if(("" == $name)||("" == $mobile_no)||("" == $pickup_point)||("" == $dropoff_point)||("" == $journey_type)||("" == $vehicle_mode)||("" == $vehicle_type)||("" == $estimated_time)||("" == $booking_datetime)||("" == $is_active)){
			$error_message = MANDATORY_MISSING;
			   ...
			   $query = "update tbl_booking_detail set fld_customer_name = '".$name."', fld_mobile_no = '".$mobile_no."'
						 ...
						 , fld_vehicle_mode = '".$vehicle_mode."', fld_dropoff_point = '".$dropoff_point."', fld_booked_driver_id = '".$driver."'
						, fld_estimated_time = '".$estimated_time."' , fld_is_active = '".$is_active."'
	...
	if("" == $id){
	  $error_message = MANDATORY_MISSING;
		...
		$query = "select customer_name, mobile_no, pickup_point, dropoff_point, journey_type, 
		          vehicle_mode, vehicle_type, estimated_time,  booking_datetime, booked_driver_id ,franchise_id, is_active, booking_ai_id 
