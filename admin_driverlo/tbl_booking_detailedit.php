...
$tbl_booking_detail_edit = NULL; // Initialize page object first

class ctbl_booking_detail_edit extends ctbl_booking_detail {
	...
	// Page object name
	var $PageObjName = 'tbl_booking_detail_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->fld_booking_ai_id->CurrentValue == "")
			$this->Page_Terminate("tbl_booking_detaillist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->fld_booking_ai_id->FldIsDetailKey)
			$this->fld_booking_ai_id->setFormValue($objForm->GetValue("x_fld_booking_ai_id"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->fld_booking_ai_id->CurrentValue = $this->fld_booking_ai_id->FormValue;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->fld_booking_ai_id->EditValue = $this->fld_booking_ai_id->CurrentValue;
			$this->fld_booking_ai_id->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($this->fld_brn_no->CurrentValue <> "") { // Check field with unique index
			$sFilterChk = "(`fld_brn_no` = '" . ew_AdjustSql($this->fld_brn_no->CurrentValue) . "')";
		...
		if ($rs->EOF) {
			...
			// fld_pickup_point
			$this->fld_pickup_point->SetDbValueDef($rsnew, $this->fld_pickup_point->CurrentValue, NULL, $this->fld_pickup_point->ReadOnly);

			// fld_customer_name
			$this->fld_customer_name->SetDbValueDef($rsnew, $this->fld_customer_name->CurrentValue, NULL, $this->fld_customer_name->ReadOnly);

			// fld_mobile_no
			$this->fld_mobile_no->SetDbValueDef($rsnew, $this->fld_mobile_no->CurrentValue, NULL, $this->fld_mobile_no->ReadOnly);

			// fld_booking_datetime
			$this->fld_booking_datetime->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->fld_booking_datetime->CurrentValue, 9), NULL, $this->fld_booking_datetime->ReadOnly);

			// fld_coupon_code
			$this->fld_coupon_code->SetDbValueDef($rsnew, $this->fld_coupon_code->CurrentValue, NULL, $this->fld_coupon_code->ReadOnly);

			// fld_driver_rating
			$this->fld_driver_rating->SetDbValueDef($rsnew, $this->fld_driver_rating->CurrentValue, 0, $this->fld_driver_rating->ReadOnly);

			// fld_customer_feedback
			$this->fld_customer_feedback->SetDbValueDef($rsnew, $this->fld_customer_feedback->CurrentValue, "", $this->fld_customer_feedback->ReadOnly);

			// fld_is_cancelled
			$this->fld_is_cancelled->SetDbValueDef($rsnew, ((strval($this->fld_is_cancelled->CurrentValue) == "1") ? "1" : "0"), 0, $this->fld_is_cancelled->ReadOnly);

			// fld_total_fare
			$this->fld_total_fare->SetDbValueDef($rsnew, $this->fld_total_fare->CurrentValue, NULL, $this->fld_total_fare->ReadOnly);

			// fld_booked_driver_id
			$this->fld_booked_driver_id->SetDbValueDef($rsnew, $this->fld_booked_driver_id->CurrentValue, NULL, $this->fld_booked_driver_id->ReadOnly);

			// fld_is_approved
			$this->fld_is_approved->SetDbValueDef($rsnew, ((strval($this->fld_is_approved->CurrentValue) == "1") ? "1" : "0"), NULL, $this->fld_is_approved->ReadOnly);

			// fld_is_completed
			$this->fld_is_completed->SetDbValueDef($rsnew, ((strval($this->fld_is_completed->CurrentValue) == "1") ? "1" : "0"), 0, $this->fld_is_completed->ReadOnly);
			...
			// fld_dropoff_point
			$this->fld_dropoff_point->SetDbValueDef($rsnew, $this->fld_dropoff_point->CurrentValue, NULL, $this->fld_dropoff_point->ReadOnly);

			// fld_estimated_time
			$this->fld_estimated_time->SetDbValueDef($rsnew, $this->fld_estimated_time->CurrentValue, NULL, $this->fld_estimated_time->ReadOnly);

			// fld_estimated_fare
			$this->fld_estimated_fare->SetDbValueDef($rsnew, $this->fld_estimated_fare->CurrentValue, NULL, $this->fld_estimated_fare->ReadOnly);

			// fld_brn_no
			$this->fld_brn_no->SetDbValueDef($rsnew, $this->fld_brn_no->CurrentValue, NULL, $this->fld_brn_no->ReadOnly);

			// fld_journey_type
			$this->fld_journey_type->SetDbValueDef($rsnew, $this->fld_journey_type->CurrentValue, NULL, $this->fld_journey_type->ReadOnly);

			// fld_vehicle_type
			$this->fld_vehicle_type->SetDbValueDef($rsnew, $this->fld_vehicle_type->CurrentValue, NULL, $this->fld_vehicle_type->ReadOnly);

			// fld_vehicle_mode
			$this->fld_vehicle_mode->SetDbValueDef($rsnew, $this->fld_vehicle_mode->CurrentValue, NULL, $this->fld_vehicle_mode->ReadOnly);
...
// Create page object
if (!isset($tbl_booking_detail_edit)) $tbl_booking_detail_edit = new ctbl_booking_detail_edit();

// Page init
$tbl_booking_detail_edit->Page_Init();

// Page main
$tbl_booking_detail_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tbl_booking_detail_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var tbl_booking_detail_edit = new ew_Page("tbl_booking_detail_edit");
tbl_booking_detail_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = tbl_booking_detail_edit.PageID; // For backward compatibility

// Form object
var ftbl_booking_detailedit = new ew_Form("ftbl_booking_detailedit");

// Validate form
ftbl_booking_detailedit.Validate = function() {
...
ftbl_booking_detailedit.Form_CustomValidate = 
...
ftbl_booking_detailedit.ValidateRequired = true;
<?php } else { ?>
ftbl_booking_detailedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftbl_booking_detailedit.Lists["x_fld_booked_driver_id"] = {"LinkField":"x_fld_driver_ai_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_fld_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $tbl_booking_detail_edit->ShowPageHeader(); ?>
<?php
$tbl_booking_detail_edit->ShowMessage();
?>
<form name="ftbl_booking_detailedit" id="ftbl_booking_detailedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="tbl_booking_detail">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table class="ewGrid"><tr><td>
<table id="tbl_tbl_booking_detailedit" class="table table-bordered table-striped">
...
<?php echo $tbl_booking_detail->fld_booking_ai_id->EditValue ?></span>
</span>
<input type="hidden" data-field="x_fld_booking_ai_id" name="x_fld_booking_ai_id" id="x_fld_booking_ai_id" value="<?php echo ew_HtmlEncode($tbl_booking_detail->fld_booking_ai_id->CurrentValue) ?>">
<?php echo $tbl_booking_detail->fld_booking_ai_id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
ew_CreateCalendar("ftbl_booking_detailedit", "x_fld_booking_datetime", "%Y/%m/%d %H:%M:%S");
</script>
<?php } ?>
</span>
...
ew_CreateCalendar("ftbl_booking_detailedit", "x_fld_created_on", "%Y/%m/%d %H:%M:%S");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
ftbl_booking_detailedit.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$tbl_booking_detail_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tbl_booking_detail_edit->Page_Terminate();
?>
