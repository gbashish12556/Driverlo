...
$tbl_driver_info_delete = NULL; // Initialize page object first

class ctbl_driver_info_delete extends ctbl_driver_info {
	...
	// Page object name
	var $PageObjName = 'tbl_driver_info_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("tbl_driver_infolist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in tbl_driver_info class, tbl_driver_infoinfo.php
...
// Create page object
if (!isset($tbl_driver_info_delete)) $tbl_driver_info_delete = new ctbl_driver_info_delete();

// Page init
$tbl_driver_info_delete->Page_Init();

// Page main
$tbl_driver_info_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tbl_driver_info_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var tbl_driver_info_delete = new ew_Page("tbl_driver_info_delete");
tbl_driver_info_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = tbl_driver_info_delete.PageID; // For backward compatibility

// Form object
var ftbl_driver_infodelete = new ew_Form("ftbl_driver_infodelete");

// Form_CustomValidate event
ftbl_driver_infodelete.Form_CustomValidate = 
...
ftbl_driver_infodelete.ValidateRequired = true;
<?php } else { ?>
ftbl_driver_infodelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftbl_driver_infodelete.Lists["x_fld_franchise_id"] = {"LinkField":"x_fld_franchise_ai_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_fld_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
if ($tbl_driver_info_delete->Recordset = $tbl_driver_info_delete->LoadRecordset())
	$tbl_driver_info_deleteTotalRecs = $tbl_driver_info_delete->Recordset->RecordCount(); // Get record count
if ($tbl_driver_info_deleteTotalRecs <= 0) { // No record found, exit
	if ($tbl_driver_info_delete->Recordset)
		$tbl_driver_info_delete->Recordset->Close();
	$tbl_driver_info_delete->Page_Terminate("tbl_driver_infolist.php"); // Return to list
}
?>
<?php $Breadcrumb->Render(); ?>
<?php $tbl_driver_info_delete->ShowPageHeader(); ?>
<?php
$tbl_driver_info_delete->ShowMessage();
?>
<form name="ftbl_driver_infodelete" id="ftbl_driver_infodelete" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="tbl_driver_info">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($tbl_driver_info_delete->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<table class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_tbl_driver_infodelete" class="ewTable ewTableSeparate">
<?php echo $tbl_driver_info->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
<?php if ($tbl_driver_info->fld_driver_ai_id->Visible) { // fld_driver_ai_id ?>
		<td><span id="elh_tbl_driver_info_fld_driver_ai_id" class="tbl_driver_info_fld_driver_ai_id"><?php echo $tbl_driver_info->fld_driver_ai_id->FldCaption() ?></span></td>
<?php } ?>
<?php if ($tbl_driver_info->fld_name->Visible) { // fld_name ?>
		<td><span id="elh_tbl_driver_info_fld_name" class="tbl_driver_info_fld_name"><?php echo $tbl_driver_info->fld_name->FldCaption() ?></span></td>
<?php } ?>
<?php if ($tbl_driver_info->fld_rating->Visible) { // fld_rating ?>
		<td><span id="elh_tbl_driver_info_fld_rating" class="tbl_driver_info_fld_rating"><?php echo $tbl_driver_info->fld_rating->FldCaption() ?></span></td>
<?php } ?>
<?php if ($tbl_driver_info->fld_is_active->Visible) { // fld_is_active ?>
		<td><span id="elh_tbl_driver_info_fld_is_active" class="tbl_driver_info_fld_is_active"><?php echo $tbl_driver_info->fld_is_active->FldCaption() ?></span></td>
<?php } ?>
<?php if ($tbl_driver_info->fld_created_on->Visible) { // fld_created_on ?>
		<td><span id="elh_tbl_driver_info_fld_created_on" class="tbl_driver_info_fld_created_on"><?php echo $tbl_driver_info->fld_created_on->FldCaption() ?></span></td>
<?php } ?>
<?php if ($tbl_driver_info->fld_license_no->Visible) { // fld_license_no ?>
		<td><span id="elh_tbl_driver_info_fld_license_no" class="tbl_driver_info_fld_license_no"><?php echo $tbl_driver_info->fld_license_no->FldCaption() ?></span></td>
<?php } ?>
<?php if ($tbl_driver_info->fld_franchise_id->Visible) { // fld_franchise_id ?>
		<td><span id="elh_tbl_driver_info_fld_franchise_id" class="tbl_driver_info_fld_franchise_id"><?php echo $tbl_driver_info->fld_franchise_id->FldCaption() ?></span></td>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$tbl_driver_info_delete->RecCnt = 0;
$i = 0;
while (!$tbl_driver_info_delete->Recordset->EOF) {
	$tbl_driver_info_delete->RecCnt++;
	$tbl_driver_info_delete->RowCnt++;

	// Set row properties
	$tbl_driver_info->ResetAttrs();
	$tbl_driver_info->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$tbl_driver_info_delete->LoadRowValues($tbl_driver_info_delete->Recordset);

	// Render row
	$tbl_driver_info_delete->RenderRow();
?>
	<tr<?php echo $tbl_driver_info->RowAttributes() ?>>
<?php if ($tbl_driver_info->fld_driver_ai_id->Visible) { // fld_driver_ai_id ?>
		<td<?php echo $tbl_driver_info->fld_driver_ai_id->CellAttributes() ?>>
<span id="el<?php echo $tbl_driver_info_delete->RowCnt ?>_tbl_driver_info_fld_driver_ai_id" class="control-group tbl_driver_info_fld_driver_ai_id">
...
<span id="el<?php echo $tbl_driver_info_delete->RowCnt ?>_tbl_driver_info_fld_name" class="control-group tbl_driver_info_fld_name">
...
<span id="el<?php echo $tbl_driver_info_delete->RowCnt ?>_tbl_driver_info_fld_rating" class="control-group tbl_driver_info_fld_rating">
...
<span id="el<?php echo $tbl_driver_info_delete->RowCnt ?>_tbl_driver_info_fld_is_active" class="control-group tbl_driver_info_fld_is_active">
...
<span id="el<?php echo $tbl_driver_info_delete->RowCnt ?>_tbl_driver_info_fld_created_on" class="control-group tbl_driver_info_fld_created_on">
...
<span id="el<?php echo $tbl_driver_info_delete->RowCnt ?>_tbl_driver_info_fld_license_no" class="control-group tbl_driver_info_fld_license_no">
...
<span id="el<?php echo $tbl_driver_info_delete->RowCnt ?>_tbl_driver_info_fld_franchise_id" class="control-group tbl_driver_info_fld_franchise_id">
...
<?php
	$tbl_driver_info_delete->Recordset->MoveNext();
}
$tbl_driver_info_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
ftbl_driver_infodelete.Init();
</script>
<?php
$tbl_driver_info_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tbl_driver_info_delete->Page_Terminate();
?>
