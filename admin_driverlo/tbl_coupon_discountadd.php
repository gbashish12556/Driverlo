...
$tbl_coupon_discount_add = NULL; // Initialize page object first

class ctbl_coupon_discount_add extends ctbl_coupon_discount {
	...
	// Page object name
	var $PageObjName = 'tbl_coupon_discount_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["fld_coupon_id"] != "") {
				...
				$this->setKey("fld_coupon_id", $this->fld_coupon_id->CurrentValue); // Set up key
			} else {
				$this->setKey("fld_coupon_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "tbl_coupon_discountview.php")
	...
	function LoadDefaultValues() {
		$this->fld_coupon_code->CurrentValue = "DefaultIsNothing";
		$this->fld_coupon_discount->CurrentValue = 0;
		$this->fld_is_validated->CurrentValue = "0";
		...
		$this->fld_is_referal->CurrentValue = "0";
	}
	...
	function AddRow($rsold = NULL) {
		...
		// fld_coupon_code
		$this->fld_coupon_code->SetDbValueDef($rsnew, $this->fld_coupon_code->CurrentValue, "", strval($this->fld_coupon_code->CurrentValue) == "");

		// fld_coupon_discount
		$this->fld_coupon_discount->SetDbValueDef($rsnew, $this->fld_coupon_discount->CurrentValue, NULL, strval($this->fld_coupon_discount->CurrentValue) == "");

		// fld_is_validated
		$this->fld_is_validated->SetDbValueDef($rsnew, ((strval($this->fld_is_validated->CurrentValue) == "1") ? "1" : "0"), NULL, strval($this->fld_is_validated->CurrentValue) == "");
		...
		// fld_is_referal
		$this->fld_is_referal->SetDbValueDef($rsnew, ((strval($this->fld_is_referal->CurrentValue) == "1") ? "1" : "0"), NULL, strval($this->fld_is_referal->CurrentValue) == "");
		...
		if ($AddRow) {
			$this->fld_coupon_id->setDbValue($conn->Insert_ID());
			$rsnew['fld_coupon_id'] = $this->fld_coupon_id->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($tbl_coupon_discount_add)) $tbl_coupon_discount_add = new ctbl_coupon_discount_add();

// Page init
$tbl_coupon_discount_add->Page_Init();

// Page main
$tbl_coupon_discount_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tbl_coupon_discount_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var tbl_coupon_discount_add = new ew_Page("tbl_coupon_discount_add");
tbl_coupon_discount_add.PageID = "add"; // Page ID
var EW_PAGE_ID = tbl_coupon_discount_add.PageID; // For backward compatibility

// Form object
var ftbl_coupon_discountadd = new ew_Form("ftbl_coupon_discountadd");

// Validate form
ftbl_coupon_discountadd.Validate = function() {
...
ftbl_coupon_discountadd.Form_CustomValidate = 
...
ftbl_coupon_discountadd.ValidateRequired = true;
<?php } else { ?>
ftbl_coupon_discountadd.ValidateRequired = false; 
<?php } ?>
...
<?php $tbl_coupon_discount_add->ShowPageHeader(); ?>
<?php
$tbl_coupon_discount_add->ShowMessage();
?>
<form name="ftbl_coupon_discountadd" id="ftbl_coupon_discountadd" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="tbl_coupon_discount">
<input type="hidden" name="a_add" id="a_add" value="A">
<table class="ewGrid"><tr><td>
<table id="tbl_tbl_coupon_discountadd" class="table table-bordered table-striped">
...
ew_CreateCalendar("ftbl_coupon_discountadd", "x_fld_created_on", "%Y/%m/%d %H:%M:%S");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
ftbl_coupon_discountadd.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$tbl_coupon_discount_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tbl_coupon_discount_add->Page_Terminate();
?>
