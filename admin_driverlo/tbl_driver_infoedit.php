...
$tbl_driver_info_edit = NULL; // Initialize page object first

class ctbl_driver_info_edit extends ctbl_driver_info {
	...
	// Page object name
	var $PageObjName = 'tbl_driver_info_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->fld_driver_ai_id->CurrentValue == "")
			$this->Page_Terminate("tbl_driver_infolist.php"); // Invalid key, return to list
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("tbl_driver_infolist.php"); // No matching record, return to list
				}
				break;
			Case "U": // Update
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->fld_driver_ai_id->FldIsDetailKey)
			$this->fld_driver_ai_id->setFormValue($objForm->GetValue("x_fld_driver_ai_id"));
		...
		if (!$this->fld_license_no->FldIsDetailKey) {
			$this->fld_license_no->setFormValue($objForm->GetValue("x_fld_license_no"));
		}
		if (!$this->fld_franchise_id->FldIsDetailKey) {
			$this->fld_franchise_id->setFormValue($objForm->GetValue("x_fld_franchise_id"));
		}
	}
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->fld_driver_ai_id->CurrentValue = $this->fld_driver_ai_id->FormValue;
		...
		$this->fld_created_on->CurrentValue = ew_UnFormatDateTime($this->fld_created_on->CurrentValue, 9);
		$this->fld_license_no->CurrentValue = $this->fld_license_no->FormValue;
		$this->fld_franchise_id->CurrentValue = $this->fld_franchise_id->FormValue;
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// fld_driver_ai_id
			$this->fld_driver_ai_id->EditCustomAttributes = "";
			$this->fld_driver_ai_id->EditValue = $this->fld_driver_ai_id->CurrentValue;
			$this->fld_driver_ai_id->ViewCustomAttributes = "";
			...
			// fld_license_no
			$this->fld_license_no->EditCustomAttributes = "";
			$this->fld_license_no->EditValue = ew_HtmlEncode($this->fld_license_no->CurrentValue);
			$this->fld_license_no->PlaceHolder = ew_HtmlEncode(ew_RemoveHtml($this->fld_license_no->FldCaption()));

			// fld_franchise_id
			$this->fld_franchise_id->EditCustomAttributes = "";
			if (trim(strval($this->fld_franchise_id->CurrentValue)) == "") {
				$sFilterWrk = "0=1";
			} else {
				$sFilterWrk = "`fld_franchise_ai_id`" . ew_SearchString("=", $this->fld_franchise_id->CurrentValue, EW_DATATYPE_NUMBER);
			}
			$sSqlWrk = "SELECT `fld_franchise_ai_id`, `fld_name` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld`, '' AS `SelectFilterFld`, '' AS `SelectFilterFld2`, '' AS `SelectFilterFld3`, '' AS `SelectFilterFld4` FROM `tbl_franchise_info`";
			$sWhereWrk = "";
			if ($sFilterWrk <> "") {
				ew_AddFilter($sWhereWrk, $sFilterWrk);
			}
			...
			$this->fld_franchise_id->EditValue = $arwrk;
	...
	function ValidateForm() {
		...
		if (!$this->fld_license_no->FldIsDetailKey && !is_null($this->fld_license_no->FormValue) && $this->fld_license_no->FormValue == "") {
			ew_AddMessage($gsFormError, $Language->Phrase("EnterRequiredField") . " - " . $this->fld_license_no->FldCaption());
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// fld_license_no
			$this->fld_license_no->SetDbValueDef($rsnew, $this->fld_license_no->CurrentValue, NULL, $this->fld_license_no->ReadOnly);

			// fld_franchise_id
			$this->fld_franchise_id->SetDbValueDef($rsnew, $this->fld_franchise_id->CurrentValue, NULL, $this->fld_franchise_id->ReadOnly);
...
// Create page object
if (!isset($tbl_driver_info_edit)) $tbl_driver_info_edit = new ctbl_driver_info_edit();

// Page init
$tbl_driver_info_edit->Page_Init();

// Page main
$tbl_driver_info_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tbl_driver_info_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var tbl_driver_info_edit = new ew_Page("tbl_driver_info_edit");
tbl_driver_info_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = tbl_driver_info_edit.PageID; // For backward compatibility

// Form object
var ftbl_driver_infoedit = new ew_Form("ftbl_driver_infoedit");

// Validate form
ftbl_driver_infoedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($tbl_driver_info->fld_name->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_fld_rating");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($tbl_driver_info->fld_rating->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_fld_rating");
			if (elm && !ew_CheckNumber(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($tbl_driver_info->fld_rating->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_fld_is_active");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($tbl_driver_info->fld_is_active->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_fld_created_on");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($tbl_driver_info->fld_created_on->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_fld_created_on");
			if (elm && !ew_CheckDate(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($tbl_driver_info->fld_created_on->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_fld_license_no");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($tbl_driver_info->fld_license_no->FldCaption()) ?>");
...
ftbl_driver_infoedit.Form_CustomValidate = 
...
ftbl_driver_infoedit.ValidateRequired = true;
<?php } else { ?>
ftbl_driver_infoedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftbl_driver_infoedit.Lists["x_fld_franchise_id"] = {"LinkField":"x_fld_franchise_ai_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_fld_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $tbl_driver_info_edit->ShowPageHeader(); ?>
<?php
$tbl_driver_info_edit->ShowMessage();
?>
<form name="ftbl_driver_infoedit" id="ftbl_driver_infoedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="tbl_driver_info">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table class="ewGrid"><tr><td>
<table id="tbl_tbl_driver_infoedit" class="table table-bordered table-striped">
...
<?php echo $tbl_driver_info->fld_driver_ai_id->EditValue ?></span>
</span>
<input type="hidden" data-field="x_fld_driver_ai_id" name="x_fld_driver_ai_id" id="x_fld_driver_ai_id" value="<?php echo ew_HtmlEncode($tbl_driver_info->fld_driver_ai_id->CurrentValue) ?>">
<?php echo $tbl_driver_info->fld_driver_ai_id->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($tbl_driver_info->fld_name->Visible) { // fld_name ?>
	<tr id="r_fld_name">
		<td><span id="elh_tbl_driver_info_fld_name"><?php echo $tbl_driver_info->fld_name->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $tbl_driver_info->fld_name->CellAttributes() ?>>
<span id="el_tbl_driver_info_fld_name" class="control-group">
<input type="text" data-field="x_fld_name" name="x_fld_name" id="x_fld_name" size="30" maxlength="50" placeholder="<?php echo $tbl_driver_info->fld_name->PlaceHolder ?>" value="<?php echo $tbl_driver_info->fld_name->EditValue ?>"<?php echo $tbl_driver_info->fld_name->EditAttributes() ?>>
</span>
<?php echo $tbl_driver_info->fld_name->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($tbl_driver_info->fld_rating->Visible) { // fld_rating ?>
	<tr id="r_fld_rating">
		<td><span id="elh_tbl_driver_info_fld_rating"><?php echo $tbl_driver_info->fld_rating->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $tbl_driver_info->fld_rating->CellAttributes() ?>>
<span id="el_tbl_driver_info_fld_rating" class="control-group">
<input type="text" data-field="x_fld_rating" name="x_fld_rating" id="x_fld_rating" size="30" placeholder="<?php echo $tbl_driver_info->fld_rating->PlaceHolder ?>" value="<?php echo $tbl_driver_info->fld_rating->EditValue ?>"<?php echo $tbl_driver_info->fld_rating->EditAttributes() ?>>
</span>
<?php echo $tbl_driver_info->fld_rating->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($tbl_driver_info->fld_is_active->Visible) { // fld_is_active ?>
	<tr id="r_fld_is_active">
		<td><span id="elh_tbl_driver_info_fld_is_active"><?php echo $tbl_driver_info->fld_is_active->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $tbl_driver_info->fld_is_active->CellAttributes() ?>>
<span id="el_tbl_driver_info_fld_is_active" class="control-group">
<div id="tp_x_fld_is_active" class="<?php echo EW_ITEM_TEMPLATE_CLASSNAME ?>"><input type="radio" name="x_fld_is_active" id="x_fld_is_active" value="{value}"<?php echo $tbl_driver_info->fld_is_active->EditAttributes() ?>></div>
<div id="dsl_x_fld_is_active" data-repeatcolumn="5" class="ewItemList">
<?php
$arwrk = $tbl_driver_info->fld_is_active->EditValue;
if (is_array($arwrk)) {
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($tbl_driver_info->fld_is_active->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " checked=\"checked\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;

		// Note: No spacing within the LABEL tag
?>
<?php echo ew_RepeatColumnTable($rowswrk, $rowcntwrk, 5, 1) ?>
<label class="radio"><input type="radio" data-field="x_fld_is_active" name="x_fld_is_active" id="x_fld_is_active_<?php echo $rowcntwrk ?>" value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?><?php echo $tbl_driver_info->fld_is_active->EditAttributes() ?>><?php echo $arwrk[$rowcntwrk][1] ?></label>
...
<?php echo $tbl_driver_info->fld_is_active->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($tbl_driver_info->fld_created_on->Visible) { // fld_created_on ?>
	<tr id="r_fld_created_on">
		<td><span id="elh_tbl_driver_info_fld_created_on"><?php echo $tbl_driver_info->fld_created_on->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $tbl_driver_info->fld_created_on->CellAttributes() ?>>
<span id="el_tbl_driver_info_fld_created_on" class="control-group">
<input type="text" data-field="x_fld_created_on" name="x_fld_created_on" id="x_fld_created_on" placeholder="<?php echo $tbl_driver_info->fld_created_on->PlaceHolder ?>" value="<?php echo $tbl_driver_info->fld_created_on->EditValue ?>"<?php echo $tbl_driver_info->fld_created_on->EditAttributes() ?>>
<?php if (!$tbl_driver_info->fld_created_on->ReadOnly && !$tbl_driver_info->fld_created_on->Disabled && @$tbl_driver_info->fld_created_on->EditAttrs["readonly"] == "" && @$tbl_driver_info->fld_created_on->EditAttrs["disabled"] == "") { ?>
...
ew_CreateCalendar("ftbl_driver_infoedit", "x_fld_created_on", "%Y/%m/%d %H:%M:%S");
</script>
<?php } ?>
</span>
<?php echo $tbl_driver_info->fld_created_on->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($tbl_driver_info->fld_license_no->Visible) { // fld_license_no ?>
	<tr id="r_fld_license_no">
		<td><span id="elh_tbl_driver_info_fld_license_no"><?php echo $tbl_driver_info->fld_license_no->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $tbl_driver_info->fld_license_no->CellAttributes() ?>>
<span id="el_tbl_driver_info_fld_license_no" class="control-group">
<input type="text" data-field="x_fld_license_no" name="x_fld_license_no" id="x_fld_license_no" size="30" maxlength="20" placeholder="<?php echo $tbl_driver_info->fld_license_no->PlaceHolder ?>" value="<?php echo $tbl_driver_info->fld_license_no->EditValue ?>"<?php echo $tbl_driver_info->fld_license_no->EditAttributes() ?>>
</span>
<?php echo $tbl_driver_info->fld_license_no->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<select data-field="x_fld_franchise_id" id="x_fld_franchise_id" name="x_fld_franchise_id"<?php echo $tbl_driver_info->fld_franchise_id->EditAttributes() ?>>
<?php
if (is_array($tbl_driver_info->fld_franchise_id->EditValue)) {
	$arwrk = $tbl_driver_info->fld_franchise_id->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($tbl_driver_info->fld_franchise_id->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
...
// Call Lookup selecting
$tbl_driver_info->Lookup_Selecting($tbl_driver_info->fld_franchise_id, $sWhereWrk);
if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
?>
<input type="hidden" name="s_x_fld_franchise_id" id="s_x_fld_franchise_id" value="s=<?php echo ew_Encrypt($sSqlWrk) ?>&amp;f0=<?php echo ew_Encrypt("`fld_franchise_ai_id` = {filter_value}"); ?>&amp;t0=19">
</span>
<?php echo $tbl_driver_info->fld_franchise_id->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
...
<script type="text/javascript">
ftbl_driver_infoedit.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$tbl_driver_info_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tbl_driver_info_edit->Page_Terminate();
?>
