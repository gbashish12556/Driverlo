...
$tbl_franchise_info_delete = NULL; // Initialize page object first

class ctbl_franchise_info_delete extends ctbl_franchise_info {
	...
	// Page object name
	var $PageObjName = 'tbl_franchise_info_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("tbl_franchise_infolist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in tbl_franchise_info class, tbl_franchise_infoinfo.php
	...
	function DeleteRows() {
		...
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$sThisKey .= $row['fld_franchise_ai_id'];
...
// Create page object
if (!isset($tbl_franchise_info_delete)) $tbl_franchise_info_delete = new ctbl_franchise_info_delete();

// Page init
$tbl_franchise_info_delete->Page_Init();

// Page main
$tbl_franchise_info_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tbl_franchise_info_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var tbl_franchise_info_delete = new ew_Page("tbl_franchise_info_delete");
tbl_franchise_info_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = tbl_franchise_info_delete.PageID; // For backward compatibility

// Form object
var ftbl_franchise_infodelete = new ew_Form("ftbl_franchise_infodelete");

// Form_CustomValidate event
ftbl_franchise_infodelete.Form_CustomValidate = 
...
ftbl_franchise_infodelete.ValidateRequired = true;
<?php } else { ?>
ftbl_franchise_infodelete.ValidateRequired = false; 
<?php } ?>
...
if ($tbl_franchise_info_delete->Recordset = $tbl_franchise_info_delete->LoadRecordset())
	$tbl_franchise_info_deleteTotalRecs = $tbl_franchise_info_delete->Recordset->RecordCount(); // Get record count
if ($tbl_franchise_info_deleteTotalRecs <= 0) { // No record found, exit
	if ($tbl_franchise_info_delete->Recordset)
		$tbl_franchise_info_delete->Recordset->Close();
	$tbl_franchise_info_delete->Page_Terminate("tbl_franchise_infolist.php"); // Return to list
}
?>
<?php $Breadcrumb->Render(); ?>
<?php $tbl_franchise_info_delete->ShowPageHeader(); ?>
<?php
$tbl_franchise_info_delete->ShowMessage();
?>
<form name="ftbl_franchise_infodelete" id="ftbl_franchise_infodelete" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="tbl_franchise_info">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($tbl_franchise_info_delete->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<table class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_tbl_franchise_infodelete" class="ewTable ewTableSeparate">
...
<?php if ($tbl_franchise_info->fld_franchise_ai_id->Visible) { // fld_franchise_ai_id ?>
		<td><span id="elh_tbl_franchise_info_fld_franchise_ai_id" class="tbl_franchise_info_fld_franchise_ai_id"><?php echo $tbl_franchise_info->fld_franchise_ai_id->FldCaption() ?></span></td>
<?php } ?>
<?php if ($tbl_franchise_info->fld_name->Visible) { // fld_name ?>
		<td><span id="elh_tbl_franchise_info_fld_name" class="tbl_franchise_info_fld_name"><?php echo $tbl_franchise_info->fld_name->FldCaption() ?></span></td>
<?php } ?>
<?php if ($tbl_franchise_info->fld_location->Visible) { // fld_location ?>
		<td><span id="elh_tbl_franchise_info_fld_location" class="tbl_franchise_info_fld_location"><?php echo $tbl_franchise_info->fld_location->FldCaption() ?></span></td>
<?php } ?>
<?php if ($tbl_franchise_info->fld_rating->Visible) { // fld_rating ?>
		<td><span id="elh_tbl_franchise_info_fld_rating" class="tbl_franchise_info_fld_rating"><?php echo $tbl_franchise_info->fld_rating->FldCaption() ?></span></td>
<?php } ?>
<?php if ($tbl_franchise_info->fld_is_active->Visible) { // fld_is_active ?>
		<td><span id="elh_tbl_franchise_info_fld_is_active" class="tbl_franchise_info_fld_is_active"><?php echo $tbl_franchise_info->fld_is_active->FldCaption() ?></span></td>
<?php } ?>
<?php if ($tbl_franchise_info->fld_created_on->Visible) { // fld_created_on ?>
		<td><span id="elh_tbl_franchise_info_fld_created_on" class="tbl_franchise_info_fld_created_on"><?php echo $tbl_franchise_info->fld_created_on->FldCaption() ?></span></td>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$tbl_franchise_info_delete->RecCnt = 0;
$i = 0;
while (!$tbl_franchise_info_delete->Recordset->EOF) {
	$tbl_franchise_info_delete->RecCnt++;
	$tbl_franchise_info_delete->RowCnt++;

	// Set row properties
	$tbl_franchise_info->ResetAttrs();
	$tbl_franchise_info->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$tbl_franchise_info_delete->LoadRowValues($tbl_franchise_info_delete->Recordset);

	// Render row
	$tbl_franchise_info_delete->RenderRow();
?>
	<tr<?php echo $tbl_franchise_info->RowAttributes() ?>>
<?php if ($tbl_franchise_info->fld_franchise_ai_id->Visible) { // fld_franchise_ai_id ?>
		<td<?php echo $tbl_franchise_info->fld_franchise_ai_id->CellAttributes() ?>>
<span id="el<?php echo $tbl_franchise_info_delete->RowCnt ?>_tbl_franchise_info_fld_franchise_ai_id" class="control-group tbl_franchise_info_fld_franchise_ai_id">
...
<span id="el<?php echo $tbl_franchise_info_delete->RowCnt ?>_tbl_franchise_info_fld_name" class="control-group tbl_franchise_info_fld_name">
...
<span id="el<?php echo $tbl_franchise_info_delete->RowCnt ?>_tbl_franchise_info_fld_location" class="control-group tbl_franchise_info_fld_location">
...
<span id="el<?php echo $tbl_franchise_info_delete->RowCnt ?>_tbl_franchise_info_fld_rating" class="control-group tbl_franchise_info_fld_rating">
...
<span id="el<?php echo $tbl_franchise_info_delete->RowCnt ?>_tbl_franchise_info_fld_is_active" class="control-group tbl_franchise_info_fld_is_active">
...
<span id="el<?php echo $tbl_franchise_info_delete->RowCnt ?>_tbl_franchise_info_fld_created_on" class="control-group tbl_franchise_info_fld_created_on">
...
<?php
	$tbl_franchise_info_delete->Recordset->MoveNext();
}
$tbl_franchise_info_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
ftbl_franchise_infodelete.Init();
</script>
<?php
$tbl_franchise_info_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tbl_franchise_info_delete->Page_Terminate();
?>
