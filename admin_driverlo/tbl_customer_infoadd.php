...
$tbl_customer_info_add = NULL; // Initialize page object first

class ctbl_customer_info_add extends ctbl_customer_info {
	...
	// Page object name
	var $PageObjName = 'tbl_customer_info_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["fld_customer_ai_id"] != "") {
				...
				$this->setKey("fld_customer_ai_id", $this->fld_customer_ai_id->CurrentValue); // Set up key
			} else {
				$this->setKey("fld_customer_ai_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "tbl_customer_infoview.php")
	...
	function LoadDefaultValues() {
		$this->fld_email->CurrentValue = "noreply@gmail.com";
		$this->fld_name->CurrentValue = NULL;
		$this->fld_name->OldValue = $this->fld_name->CurrentValue;
		$this->fld_mobile_no->CurrentValue = NULL;
		$this->fld_mobile_no->OldValue = $this->fld_mobile_no->CurrentValue;
		$this->fld_password->CurrentValue = "defaultStringIfNothingFound";
		$this->fld_rating->CurrentValue = 0.00;
		$this->fld_is_active->CurrentValue = "1";
		$this->fld_is_blocked->CurrentValue = "0";
		$this->fld_created_on->CurrentValue = NULL;
		$this->fld_created_on->OldValue = $this->fld_created_on->CurrentValue;
		$this->fld_total_point->CurrentValue = 200;
		$this->fld_referal_code->CurrentValue = "defaultStringIfNothingFound";
	}
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->fld_mobile_no->CurrentValue <> "") { // Check field with unique index
			$sFilter = "(fld_mobile_no = '" . ew_AdjustSql($this->fld_mobile_no->CurrentValue) . "')";
		...
		// fld_email
		$this->fld_email->SetDbValueDef($rsnew, $this->fld_email->CurrentValue, NULL, strval($this->fld_email->CurrentValue) == "");

		// fld_name
		$this->fld_name->SetDbValueDef($rsnew, $this->fld_name->CurrentValue, NULL, FALSE);

		// fld_mobile_no
		$this->fld_mobile_no->SetDbValueDef($rsnew, $this->fld_mobile_no->CurrentValue, NULL, FALSE);

		// fld_password
		$this->fld_password->SetDbValueDef($rsnew, $this->fld_password->CurrentValue, NULL, strval($this->fld_password->CurrentValue) == "");
		...
		// fld_is_blocked
		$this->fld_is_blocked->SetDbValueDef($rsnew, ((strval($this->fld_is_blocked->CurrentValue) == "1") ? "1" : "0"), NULL, strval($this->fld_is_blocked->CurrentValue) == "");

		// fld_created_on
		$this->fld_created_on->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->fld_created_on->CurrentValue, 9), NULL, FALSE);

		// fld_total_point
		$this->fld_total_point->SetDbValueDef($rsnew, $this->fld_total_point->CurrentValue, NULL, strval($this->fld_total_point->CurrentValue) == "");

		// fld_referal_code
		$this->fld_referal_code->SetDbValueDef($rsnew, $this->fld_referal_code->CurrentValue, NULL, strval($this->fld_referal_code->CurrentValue) == "");
		...
		if ($AddRow) {
			$this->fld_customer_ai_id->setDbValue($conn->Insert_ID());
			$rsnew['fld_customer_ai_id'] = $this->fld_customer_ai_id->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($tbl_customer_info_add)) $tbl_customer_info_add = new ctbl_customer_info_add();

// Page init
$tbl_customer_info_add->Page_Init();

// Page main
$tbl_customer_info_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tbl_customer_info_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var tbl_customer_info_add = new ew_Page("tbl_customer_info_add");
tbl_customer_info_add.PageID = "add"; // Page ID
var EW_PAGE_ID = tbl_customer_info_add.PageID; // For backward compatibility

// Form object
var ftbl_customer_infoadd = new ew_Form("ftbl_customer_infoadd");

// Validate form
ftbl_customer_infoadd.Validate = function() {
...
ftbl_customer_infoadd.Form_CustomValidate = 
...
ftbl_customer_infoadd.ValidateRequired = true;
<?php } else { ?>
ftbl_customer_infoadd.ValidateRequired = false; 
<?php } ?>
...
<?php $tbl_customer_info_add->ShowPageHeader(); ?>
<?php
$tbl_customer_info_add->ShowMessage();
?>
<form name="ftbl_customer_infoadd" id="ftbl_customer_infoadd" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="tbl_customer_info">
<input type="hidden" name="a_add" id="a_add" value="A">
<table class="ewGrid"><tr><td>
<table id="tbl_tbl_customer_infoadd" class="table table-bordered table-striped">
...
ew_CreateCalendar("ftbl_customer_infoadd", "x_fld_created_on", "%Y/%m/%d %H:%M:%S");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
ftbl_customer_infoadd.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$tbl_customer_info_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$tbl_customer_info_add->Page_Terminate();
?>
