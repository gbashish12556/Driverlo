...
function CurrentProjectID() {
	...
	return "{EF683EA7-113B-4FD4-A0F1-4B1B7BEBED8D}";
}
	...
	function ValidateUser(&$usr, &$pwd, $autologin) {
		...
		if (!$ValidateUser) {
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					$ValidateUser = $CustomValidateUser || ew_ComparePassword($rs->fields('fld_password'), $pwd);
					if ($ValidateUser) {
						$_SESSION[EW_SESSION_STATUS] = "login";
						$_SESSION[EW_SESSION_SYS_ADMIN] = 0; // Non System Administrator
						$this->setCurrentUserName($rs->fields('fld_user_name')); // Load user name
						if (is_null($rs->fields('fld_level'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('fld_level'))); // Load User Level
						}
						$this->SetUpUserLevel();
