...
function get_citylist($select_vehicle = "")
{
    ...
    $query = "SELECT city_name, city_id FROM view_fare_chart WHERE is_active = '1' ";
	//echo $query;
...
function IsCouponValid($coupon_code){
	$query_function = "select coupon_code from view_coupon_discount where coupon_code='".$coupon_code."' and is_validated = '0' and is_referal = '0' ";
	if($result_function = loader_query($query_function)){
	     if(loader_num_rows($result_function)>0){
