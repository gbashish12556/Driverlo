...
if(loader_post_isset('pickup_point')){
	...
	if(("" == $name)||("" == $mobile_no)||("" == $pickup_point)||("" == $booking_datetime)){
		...
		if(!loader_session_isset('mobile_no')){
			...
			$bcn_no = 'BRN'.time();
			$customer_token = loader_get_session('user_token');
			$message = "Booked Successfully.Congratulations !! Booking receipt note for tracking is ".$bcn_no."";
			$query1 = "INSERT INTO tbl_booking_detail (fld_customer_token, fld_customer_name,fld_mobile_no,fld_pickup_point,fld_booking_datetime,fld_coupon_code,fld_brn_no) VALUES('".$customer_token."', '".$name."','".$mobile_no."','".$pickup_point."','".$booking_datetime."','".$coupon_code."','".$bcn_no."')";
	...
	{		
			...
			if(($error_message == "")){
					...
					$bcn_no = 'BRN'.time();
					$message = "Booked Successfully.Congratulations !! Booking receipt note for tracking is ".$bcn_no."";
					$query1 = "INSERT INTO tbl_booking_detail (fld_customer_token, fld_customer_name,fld_mobile_no,fld_pickup_point,fld_booking_datetime,fld_coupon_code,fld_brn_no) VALUES('".$user_token."', '".$name."','".$mobile_no."','".$pickup_point."','".$booking_datetime."','".$coupon_code."','".$bcn_no."')";
					if(loader_query($query1)){
						$success_message .= $message;
						$approval_status = "Approval Pending";
						$booking_datetime = date('d/m/Y g:i a', strtotime($booking_datetime));		
	...
	$name = loader_get_session('name');
  	  $query = "select customer_name, mobile_no,brn_no, pickup_point, booking_datetime, coupon_code, is_approved from view_booking_detail where mobile_no = '".$mobile_no."' order by created_on desc LIMIT 1";
